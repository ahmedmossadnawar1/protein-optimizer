    ...
    try:
        ...
        if weights is None:
            ...
            for i, f in enumerate(foods):
                group = f['group'].lower()
                prot = food_proteins[i]
                
                if prot < 10.0:
                    continue
                if not any(p in group for p in ['legume', 'seed', 'nut', 'grain', 'cereals']):
                    continue
                
                valid_candidates.append((prot, i))
                
            if len(valid_candidates) == 0:
                for i, f in enumerate(foods):
                    if food_proteins[i] >= 10.0:
                        valid_candidates.append((food_proteins[i], i))
            if len(valid_candidates) == 0:
                for i, f in enumerate(foods):
                    valid_candidates.append((food_proteins[i], i))
            ...
            num_ingred = min(len(valid_candidates), 5)
            if num_ingred > 3 and len(valid_candidates) >= 4:
                num_ingred = min(len(valid_candidates), 4)
            subset = valid_candidates[:num_ingred]
            
            weights = np.zeros(len(foods))
            if num_ingred == 1:
                weights[subset[0][1]] = 1.0
            elif num_ingred == 2:
                weights[subset[0][1]] = 0.60
                weights[subset[1][1]] = 0.40
            elif num_ingred == 3:
                weights[subset[0][1]] = 0.50
                weights[subset[1][1]] = 0.30
                weights[subset[2][1]] = 0.20
            elif num_ingred == 4:
                weights[subset[0][1]] = 0.45
                weights[subset[1][1]] = 0.25
                weights[subset[2][1]] = 0.15
                weights[subset[3][1]] = 0.15
            else:
                dist = [0.40, 0.25, 0.15, 0.10, 0.10]
                for i in range(5):
                    weights[subset[i][1]] = dist[i]
